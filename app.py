...
def index():
    if request.method == 'POST':
        try:
            searchstring = request.form["content"].replace(" ","") # entering the input
            ...
            for i in data:
                try:
                    new_url = "https://www.flipkart.com" + (i.find("a", {"class": "_31qSD5"})["href"])  #finding the url for each product displayes after the search
                    req_newurl_soup = bs(requests.get(new_url).content, "html.parser")
                except Exception as q:
                    print("The error is : ",q)
                    break
                try:
                    review_url = "https://www.flipkart.com" + (req_newurl_soup.find("div", {"class": "swINJg _3nrCtb"}).find_parent().get("href"))  #code for finding the entire review page for each product
